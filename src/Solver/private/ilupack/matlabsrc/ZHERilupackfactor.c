...
)
{
    ...
    integer    *ibuff, *istack, blocksize, ii, *Ibuff,jj;
    size_t     sizebuf;
    ...
    doublecomplex *mytv, *Dbuff, *dbuff;
    ...
    */
    ...
    if (tv_exists && tv_field>=0 && 
       ...
       pr=mxGetPr(tmp);
       mytv=(doublecomplex *)MAlloc((size_t)A.nr*sizeof(doublecomplex),"ZHERilupackfactor");
       param->tv=mytv;
       if (!mxIsComplex(tmp)) {
	  for (i=0; i<A.nr; i++) {
	      param->tv[i].r=pr[i];
...
#ifdef PRINT_INFO
    mexPrintf("ZHERilupackfactor: factorize matrix\n");fflush(stdout);
#endif
    if (nrhs==4) {
...
#ifdef PRINT_INFO
    mexPrintf("ZHERilupackfactor: matrix factored\n");fflush(stdout);
    if (param->rcomflag!=0) {
       mexPrintf("ZHERilupackfactor: request for reverse communication\n");fflush(stdout);
    }
#endif
    ...
    /* ibuff =(integer*)      MAlloc((size_t)n*sizeof(integer),
				  "ZHERilupackfactor:ibuff");
    */
    istack=(integer*)      MAlloc((size_t)n*sizeof(integer),
				  "ZHERilupackfactor:istack");
    /* dbuff =(doublecomplex*)MAlloc((size_t)n*sizeof(doublecomplex),
				  "ZHERilupackfactor:dbuff");
    */
