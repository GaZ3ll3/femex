...
)
{
    ...
    integer    *ibuff, *Ibuff, ii, blocksize, *istack,jj;
    size_t     sizebuf;
    ...
    */
    ...
    param=(ZILUPACKparam *)MAlloc((size_t)sizeof(ZILUPACKparam),"ZSYMilupackfactor:param");
    ZSYMAMGinit(&A,param);
    ...
    mexPrintf("ZSYMilupackfactor: copy shifts\n");fflush(stdout);
	      ...
	      param->shifts=(doublecomplex *)MAlloc(param->nshifts*sizeof(doublecomplex),
						    "ZSYMilupackfactor");
	      pr=mxGetPr(tmp);
	      pi=mxGetPi(tmp);
    ...
    mexPrintf("ZSYMilupackfactor: copy shift matrix\n");fflush(stdout);
		 ...
		 mexErrMsgTxt("Shift matrix must be a square matrix.");
	      ...
	      /* copy input matrix to sparse row format */
	      M.nc=M.nr=mrows;
	      M.ia=(integer *) MAlloc((size_t)(M.nc+1)*sizeof(integer),"ZSYMilupackfactor");
	      M.ja=(integer *) MAlloc((size_t)nnz     *sizeof(integer),"ZSYMilupackfactor");
	      M. a=(doublecomplex *)MAlloc((size_t)nnz*sizeof(doublecomplex), "ZSYMilupackfactor");
    ...
    if (tv_exists && tv_field>=0 && 
       ...
       pr=mxGetPr(tmp);
       mytv=(doublecomplex *)MAlloc((size_t)A.nr*sizeof(doublecomplex),"ZSYMilupackfactor");
       param->tv=mytv;
       if (!mxIsComplex(tmp)) {
	  for (i=0; i<A.nr; i++) {
	      param->tv[i].r=pr[i];
    ...
    PRE=(ZAMGlevelmat *)MAlloc((size_t)sizeof(ZAMGlevelmat),"ZSYMilupackfactor");
    ...
    mexPrintf("ZSYMilupackfactor: return shift matrix\n");fflush(stdout);
	   ...
	   if (A_valuesI==NULL) {
	      ...
	      fout=mxCreateSparse((mwSize)mrows,(mwSize)ncols, (mwSize)nnz, mxCOMPLEX);
    ...
    istack =(integer*)MAlloc((size_t)n*sizeof(integer),"ZSYMilupackfactor:istack");
    convert=(double*) MAlloc((size_t)n*sizeof(double), "ZSYMilupackfactor:convert");
	...
	if (param->rcomflag==0) {
	   ...
	   if (jstruct==PRE->nlev-1 && 
		 ...
		 Dbuff=(doublecomplex *)MAlloc((size_t)2*m*m*sizeof(doublecomplex),"ZSYMilupackfactor");
		 Ibuff=(integer *)MAlloc((size_t)m*sizeof(integer),"ZSYMilupackfactor");
		 ...
		 if (PRE->issingle) {
		    for (i=0; i<scurrent->nB; ) {
			   ...
			   deti=scurrent->LU.a[i].r*scurrent->LU.a[i+1].i+scurrent->LU.a[i].i*scurrent->LU.a[i+1].r
			       -scurrent->LU.a[scurrent->nB+1+i].r*scurrent->LU.a[scurrent->nB+1+i].i-scurrent->LU.a[scurrent->nB+1+i].i*scurrent->LU.a[scurrent->nB+1+i].r;
			   ...
			   */
			   ...
			   si[k++]=det*(detr*scurrent->LU.a[i+1].i-deti*scurrent->LU.a[i+1].r);
			   ...
			   si[k++]=det*(detr*scurrent->LU.a[i].i-deti*scurrent->LU.a[i].r);
		    ...
		    for (i=0; i<current->nB; ) {
			   ...
			   deti=current->LU.a[i].r*current->LU.a[i+1].i+current->LU.a[i].i*current->LU.a[i+1].r
			       -current->LU.a[current->nB+1+i].r*current->LU.a[current->nB+1+i].i-current->LU.a[current->nB+1+i].i*current->LU.a[current->nB+1+i].r;
			   /* 1/determinant = conjug(determinant)/|determinant|^2 */
			   det=1.0/(detr*detr+deti*deti);
			   ...
			   */
			   ...
			   si[k++]=det*(detr*current->LU.a[i+1].i-deti*current->LU.a[i+1].r);
			   ...
			   si[k++]=det*(detr*current->LU.a[i].i-deti*current->LU.a[i].r);
	...
	if (jstruct>=PRE->nlev-1) {
	   ...
	   if (jstruct==PRE->nlev-2 && 
		 ...
		 for (i=0; i<scurrent->next->A.nr; i++) {
		     ...
		     Cqsort(scurrent->next->A.a+j,scurrent->next->A.ja+j,istack,&jj);
		 ...
		 for (i=0; i<current->next->A.nr; i++) {
		     ...
		     Zqsort(current->next->A.a+j,current->next->A.ja+j,istack,&jj);
