...
! implementation for real matrices in double_precision
...
!
    ...
    IF (preprocessed) THEN
       CALL dagmg_relmem
       IF (.NOT.allzero) CALL dagmg_MUMPSseq(nn(nlev),a,ja,ia,f,-2,flop)
       preprocessed=.FALSE.
       solve=.FALSE.
       eltm=0.0d0
       cputm=0.0d0
...
!      DIRECT SOLVE IF ONLY ONE LEVEL
   ...
   !
       x(1:n)=f(1:n)
       CALL dagmg_MUMPSseq(n,a,ja,ia,x,2,flop)
       mritr=0
...
500 CONTINUE
    CALL dagmg_relmem
    IF (.NOT.allzero) CALL dagmg_MUMPSseq(n,a,ja,ia,f,-2,flop)
    preprocessed=.FALSE.
    solve=.FALSE.
    eltm=0.0d0
    cputm=0.0d0
...
!-----------------------------------------------------------------------
  SUBROUTINE dagmg_csrdlu(n,a,ja,ia,idiag,iext,ao,jao,il,iu,iexto,trans)
!
...
!-----------------------------------------------------------------------
  SUBROUTINE dagmg_FlexCG(N,f,X,ITER,RESID,a,ja,ia,init,flop)
    ...
    !
    IF (init==1) THEN
       CALL dagmg_matv(N,x,S(1+N),a,ja,ia,flop,iextL1,       &
    ...
    DO WHILE ( ITER < MAXIT .AND. RESID > TOL )
       ...
       !
       IF ( ITER > 1 ) THEN
          dum(1) = - DDOT(NBLAP, S(1+N), IONE, S(1+3*N), IONE)
          BET0=dum(1)
          BET0=BET0/RHO
          CALL DAXPY(NBLAP, BET0, S(1+2*N), IONE, S(1+3*N), IONE)
          flop=flop+dble(4*N)
       ENDIF
       CALL DCOPY(NBLAP, S(1+3*N), IONE, S(1+2*N), IONE)
       CALL dagmg_matv(N,S(1+2*N),S(1+N),a,ja,ia,flop,iextL1,   &
            dt(1)%lstout,dt(1)%ilstout,dt(1)%lstin,dt(1)%ilstin,trans)
       !
       dum(1) =  DDOT(NBLAP, S(1+2*N), IONE, S(1+N), IONE)
       ...
       ELSE
          ...
          flop=flop+dble(2*N)
       END IF
       CALL DAXPY(NBLAP, -ALPHA, S(1+N), IONE, f, IONE)
       !
...
!-----------------------------------------------------------------------
  SUBROUTINE dagmg_GCR(N,f,X,ITER,RESID,a,ja,ia,init,flop)
    ...
    REAL(kind(0.0d0)), ALLOCATABLE :: S(:), R(:),fsc(:)
    REAL(kind(0.0d0)), external :: DDOT
    REAL(kind(0.0d0)), external :: DNRM2
    INTEGER (KIND=selected_int_kind(12)), parameter :: IONE=1
    ...
    ALLOCATE (S(n+1:N*(2*nrst+3)+((nrst+1)*nrst)/2+nrst),R(nwrkcum))
    !
    flop=0.0d0
    kstat=0
    IF (wfo) THEN
    ...
    DO WHILE ( ITER < MAXIT .AND. RESID2 > TOL2BNORM2 )
       ...
       IF (itm == m) THEN
          ...
          IF (irst == 0 .AND. init == 0) THEN
             CALL DGEMV('N',NBLAP,m,1.0d0,S(1+ku*N), &
                  NBLAP,S(1+iv+kfree*N),IONE,0.0d0,&
                  X,IONE)
       ...
       !
       CALL dagmg_prec_matv(1,f,S(1+(ku+itm)*N),S(1+(kc+itm)*N)  &
            ,flop,R,.FALSE.)
       ...
       IF (itm > 0) THEN
          DO i=0,itm-1
             dum(1)=DDOT(NBLAP,S(1+(kc+i)*N),IONE,S(1+(kc+itm)*N),IONE)
             bet0=dum(1)
             bet0=bet0/S(1+iR+i+(i*(i+1))/2+kfree*N)
             S(1+iR+i+(itm*(itm+1))/2+kfree*N)=bet0
             CALL DAXPY(NBLAP,-bet0,S(1+(kc+i)*N),IONE,      &
                  S(1+(kc+itm)*N),IONE)
             flop=flop+dble(4*N)
          END DO
       END IF
       ...
       !
       dum(1)=DNRM2(NBLAP,S(1+(kc+itm)*N),IONE)**2
       dum(2)=DDOT(NBLAP, S(1+(kc+itm)*N), IONE, f, IONE)
       IF (ITER == 1) THEN
          IF (init == 0) THEN
             BNORM2=dum(3)
             RESID2=BNORM2
       ...
       !
       CALL DAXPY(NBLAP, -bet0, S(1+(kc+itm)*N), IONE, f, IONE)
       flop=flop+dble(6*N)
       !
    ...
    IF (itm >= 0) THEN
       ...
       CALL DTPTRS('U','N','U',itm1, IONE,S(1+iR+kfree*N),     &
            S(1+iv+kfree*N),m,info)
       IF (irst == 0 .AND. init == 0) THEN
          CALL DGEMV('N',NBLAP,itm1,1.0d0,S(1+ku*N), &
               NBLAP,S(1+iv+kfree*N),IONE,0.0d0,   &
               X,IONE)
...
!
    ...
    IF (nnext > 0) THEN
       ...
       ELSE
          ...
          CALL dagmg_Galerkin_inner(nnext,R(XN),R(BN),l+1,flop)
          !
       END IF
       !
...
!   stored in (dt(l)%a,dt(l)%ja,dt(l)%iu) (strict upper part)
    ...
    IF (update) THEN
       ...
       !
       CALL  dagmg_csrmv(n,dt(l)%a,dt(l)%ja,n+1,dt(l)%iu,R,1,AX,-1,flop,-1)
       !
    ELSE
       !
       ! Solve (D+E) X = B
       ...
       ! Hence, compute AX = - F * R
       !
       CALL  dagmg_csrmv(n,dt(l)%a,dt(l)%ja,n+1,dt(l)%iu,X,1,AX,-1,flop,-1)
       !
    END IF
    RETURN
...
!
    ...
    !
    AX(1:n)=B(1:n)
    CALL  dagmg_csrmv(n,dt(l)%a,dt(l)%ja,n+1,dt(l)%il,X,1,AX,-2,flop,-1)
    !
    ! Solve (D+F) X = AX
...
!
    ...
    !
    CALL  dagmg_csrmv(n,dt(l)%a,dt(l)%ja,n+1,dt(l)%iu,X,1,AX,0,flop,-1)
    !
    ! Then, R(1:n)= B - AX
    R(1:n)=B(1:n)-AX(1:n)
...
!
    ...
    IF (1 /= nlev) THEN
       ...
       !
       CALL dagmg_aggregation(1,n,a,ja,ia,nc,listrank)
       !
       ! Perform setup at next level
       !
       CALL dagmg_setup(2,nc,listrank,nc+1)
       !
    ELSE
       !
    ...
    !
       ...
       !
       CALL dagmg_csrdlu(n,a,ja,ia,dt(1)%idiag,iextL1               &
...
!-----------------------------------------------------------------------
  RECURSIVE SUBROUTINE dagmg_setup(l,n,listrank,ifl)
!
...
!         .OR. ( zerors .AND. ngl(1) <= maxcoarseslowt )  &
    ...
    IF (l /= nlev) THEN
       ...
       ! Coarsening by aggregation
       !
       CALL dagmg_aggregation(l,n,dt(l)%a,dt(l)%ja,dt(l)%ia,nc,listrank)
       !
       ! Perform setup at next level
       !
       CALL dagmg_setup(l+1,nc,listrank,nc+1)
       !
    ELSE
       !
       ! Coarsest level set up
    ...
    !
       ...
       memax=MAX(memax,memr+memi*rlenilen)
       !
       CALL dagmg_csrdlu(n,dt(l)%a,dt(l)%ja,dt(l)%ia,dt(l)%idiag,dt(l)%iext &
...
!
    ...
    IF (l .EQ. 1) THEN
       ...
       memi=memi+4*n+1
       memr=memr+n
       CALL dagmg_setCMK(n,ja,ia,dt(l)%idiag,dt(l)%iext,riperm,iperm)
    ELSE
    ...
    memax=MAX(memax,memr+memi*rlenilen)
    !
    call dagmg_prepareagg(n,a,ja,ia,dt(l)%idiag,dt(l)%iext,ind2   &
            ,iperm,si1,ndd,l)
    !
    IF (ndd .EQ. n) THEN
       nc=0
    ...
    IF (l > 1) THEN
       IF (spd) THEN
          CALL dagmg_findpairs_SI(n,a,ja,ia,dt(l)%idiag,dt(l)%iext,si1  &
               ,ind2,lcg,nc,ndd,ldd,skipass                              &
               ,iperm)
       ELSE
          CALL dagmg_findpairs_GI(n,a,ja,ia,dt(l)%idiag,dt(l)%iext,si1  &
               ,ind2,lcg,nc,ndd,ldd,skipass                              &
               ,iperm)
       END IF
       DEALLOCATE(iperm)
    ...
    ELSE
       IF (spd) THEN
          CALL dagmg_findpairs_SI1(n,a,ja,ia,dt(l)%idiag,dt(l)%iext,si1  &
               ,ind2,lcg,nc,ndd,ldd,skipass                               &
               ,riperm,iperm)
       ELSE
          CALL dagmg_findpairs_GI1(n,a,ja,ia,dt(l)%idiag,dt(l)%iext,si1  &
               ,ind2,lcg,nc,ndd,ldd,skipass                               &
               ,riperm,iperm)
       END IF
    ...
    memax=MAX(memax,memr+memi*rlenilen)
    CALL dagmg_setcg(n,a,ja,ia,dt(l)%idiag,dt(l)%iext,si1,ind2,lcg      &
    ...
    DO kpass=2,npass1
       ...
       ind2(1:np)=-1
       IF (spd) THEN
          CALL dagmg_findpairs_SF(np,ap,jap,iap,idiagp,iextp,sip  &
               ,ind2,lcg,nc                                        &
               ,m1,lcg1,a,ja,ia,dt(l)%idiag,dt(l)%iext,si1,w,iw)
       ELSE
          CALL dagmg_findpairs_GF(np,ap,jap,iap,idiagp,iextp,sip     &
               ,ind2,lcg,nc                                           &
               ,m1,lcg1,a,ja,ia,dt(l)%idiag,dt(l)%iext,si1,w,iw)
       END IF
       DEALLOCATE(w,iw)
       ...
       !
       CALL dagmg_setcg(np,ap,jap,iap,idiagp,iextp,sip,ind2,lcg,nc,an     &
...
!-----------------------------------------------------------------------
  SUBROUTINE dagmg_setCMK(n,ja,ia,idiag,iext,riperm,iperm)
    !
...
!-----------------------------------------------------------------------
  SUBROUTINE dagmg_prepareagg(n,a,ja,ia,idiag,iext,ind2,iperm,si,ndd,l)
    !
...
!-----------------------------------------------------------------------
  SUBROUTINE dagmg_findpairs_GF(n,a,ja,ia,idiag,iext,si,ind,lcg,nc     &
...
20     CONTINUE
       ...
       !       in right order
       CALL dagmg_checktentagg_GF(isel,ipair           &
...
!-----------------------------------------------------------------------
  SUBROUTINE dagmg_findpairs_SF(n,a,ja,ia,idiag,iext,si,ind,lcg,nc     &
    ,m1,lcg1,a1,ja1,ia1,idiag1,iext1,si1,rtent,jtent)
! Performs pairwise aggregation according to Algorithms 4.2 and 4.3 in [2,3],
! with some heuristic enhancements to cover non diagonally  dominant matrices.
!
...
20     CONTINUE
       !       Perform check in unagregated matrix, considering possible pairs
       !       in right order
       CALL dagmg_checktentagg_SF(isel,ipair           &
            ,m1,lcg1,a1,ja1,ia1,idiag1,iext1,si1,val,acc)
       !
       IF (.NOT.acc) THEN
          !     Tentative pair has been rejected, check for the next one, if any
...
!-----------------------------------------------------------------------
  SUBROUTINE dagmg_findpairs_GI(n,a,ja,ia,idiag,iext,si,ind,lcg,nc     &
    ,nddl,ldd,skipass                                      &
    ,ipc)
! Performs pairwise aggregation according to Algorithms 4.2 and 4.3 in [2,3],
! with some heuristic enhancements to cover non diagonally  dominant matrices.
...
!-----------------------------------------------------------------------
  SUBROUTINE dagmg_findpairs_SI(n,a,ja,ia,idiag,iext,si,ind,lcg,nc     &
    ,nddl,ldd,skipass                                      &
    ,ipc)
! Performs pairwise aggregation according to Algorithms 4.2 and 4.3 in [2,3],
! with some heuristic enhancements to cover non diagonally  dominant matrices.
...
!-----------------------------------------------------------------------
  SUBROUTINE dagmg_findpairs_GI1(n,a,ja,ia,idiag,iext,si,ind,lcg,nc     &
    ,nddl,ldd,skipass                                      &
    ,riperm,iperm)
! Performs pairwise aggregation according to Algorithms 4.2 and 4.3 in [2,3],
! with some heuristic enhancements to cover non diagonally  dominant matrices.
...
!-----------------------------------------------------------------------
  SUBROUTINE dagmg_findpairs_SI1(n,a,ja,ia,idiag,iext,si,ind,lcg,nc     &
    ,nddl,ldd,skipass                                      &
    ,riperm,iperm)
! Performs pairwise aggregation according to Algorithms 4.2 and 4.3 in [2,3],
! with some heuristic enhancements to cover non diagonally  dominant matrices.
...
!-----------------------------------------------------------------------
  SUBROUTINE dagmg_setcg(n,a,ja,ia,idiag,iext,si,ind,lcg           &
