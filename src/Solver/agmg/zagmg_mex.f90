...
! implementation for complex matrices in double_precision
...
!-----------------------------------------------------------------------
!!!!!!!!!!!!!!!!!!! MAIN DRIVER and associated routines
!-----------------------------------------------------------------------
...
!
    ...
    REAL(kind(0.0d0)) :: cputmp,eltmp,resid,memh
...
!
    ...
    IF (preprocessed) THEN
       CALL zagmg_relmem
       IF (.NOT.allzero) CALL zagmg_MUMPSseq(nn(nlev),a,ja,ia,f,-2,flop)
       preprocessed=.FALSE.
       solve=.FALSE.
       eltm=0.0d0
       cputm=0.0d0
...
!
    ...
    !
    ! Complex case: first perform
    !    diagonal scaling such that diagonal elements become real
    !
    ALLOCATE(scald(n),dt(1)%a(ia(n+1)-ia(1)))
    memr=memr+n+ia(n+1)-ia(1)
    CALL zagmg_diagscal(n,a,ja,ia,dt(1)%idiag,dt(1)%a,scald)
    CALL zagmg_setupL1(n,dt(1)%a,ja,ia,listrank,ifirstlistrank)
    !
...
!      DIRECT SOLVE IF ONLY ONE LEVEL
    IF (nlev == 1) THEN
       ...
       END IF
   !
       x(1:n)=f(1:n)*scald(1:n)
       CALL zagmg_MUMPSseq(n,a,ja,ia,x,2,flop)
       x(1:n)=f(1:n)*scald(1:n)
       mritr=0
       IF (MOD(ijb,10) >= 2) RETURN
       GOTO 500
   !
   !      END OF DIRECT SOLVE
   !
   !-     SINGLE APPLICATION OF MG PRECONDITIONER
    ELSE IF (MOD(ijb,10) >= 3) THEN
       IF (wfo) THEN
...
455 CONTINUE
    IF (wfo) THEN
       ...
       END IF
       call mexPrintf(char(10))
       WRITE(lout,954) IRANK,flop/dble(11*n+2*nza)
       call mexPrintf(lout//char(10))
...
500 CONTINUE
    CALL zagmg_relmem
    IF (.NOT.allzero) CALL zagmg_MUMPSseq(n,a,ja,ia,f,-2,flop)
    preprocessed=.FALSE.
    solve=.FALSE.
    eltm=0.0d0
    cputm=0.0d0
...
!-----------------------------------------------------------------------
  SUBROUTINE zagmg_diagscal(n,a,ja,ia,idiag,as,scald)
    IMPLICIT NONE
    INTEGER :: n, ja(*), ia(n+1), idiag(n)
    COMPLEX(kind(0.0d0)) :: a(*) ,as(*), scald(*), val
    INTEGER :: i,k
    DO i=1,n
       val=a(idiag(i))
...
!-----------------------------------------------------------------------
  SUBROUTINE zagmg_csrdlu(n,a,ja,ia,idiag,iext,ao,jao,il,iu,iexto,trans)
!
...
!
    !
    IMPLICIT NONE
    INTEGER :: n, ifja, ja(ifja:*), ia(n+1), lstin(0:*), iad, ifx, i, k, j
    COMPLEX(kind(0.0d0)) :: a(*), x(ifx:*), y(n), t
    REAL(kind(0.0d0)) :: flop
    !
    IF ( lstin(0) .LT. 0) THEN
       !
       IF (iad .LT. -1) THEN
          DO i=1,n
             t=y(i)
...
!-----------------------------------------------------------------------
  SUBROUTINE zagmg_FlexCG(N,f,X,ITER,RESID,a,ja,ia,init,flop)
    ...
    COMPLEX(kind(0.0d0)) :: ALPHA, BET0, RHO, dum(6)
    ...
    !
    mritr=nwrkcum+5*N
    ALLOCATE (S(n+1:nwrkcum+5*N),fsc(N))
    !
    flop=0.0d0
    kstat=0
    IF (wfo) THEN
    ...
    !
    IF (init==1) THEN
       CALL zagmg_matv(N,x,S(1+N),a,ja,ia,flop,iextL1,       &
            dt(1)%lstout,dt(1)%ilstout,dt(1)%lstin,dt(1)%ilstin,trans)
       f(1:n)=f(1:n)-S(1+N:2*N)
    ...
    DO WHILE ( ITER < MAXIT .AND. RESID > TOL )
       ...
       !
       IF ( ITER > 1 ) THEN
          dum(1) = - ZDOTC(NBLAP, S(1+N), IONE, S(1+3*N), IONE)
          BET0=dum(1)
          BET0=BET0/RHO
          CALL ZAXPY(NBLAP, BET0, S(1+2*N), IONE, S(1+3*N), IONE)
          flop=flop+dble(4*N)
       ENDIF
       CALL ZCOPY(NBLAP, S(1+3*N), IONE, S(1+2*N), IONE)
       CALL zagmg_matv(N,S(1+2*N),S(1+N),a,ja,ia,flop,iextL1,   &
            dt(1)%lstout,dt(1)%ilstout,dt(1)%lstin,dt(1)%ilstin,trans)
       !
       dum(1) =  ZDOTC(NBLAP, S(1+2*N), IONE, S(1+N), IONE)
       dum(2) =  ZDOTC(NBLAP,S(1+2*N),IONE,f,IONE)
       flop=flop+dble(4*N)
       !
       IF (ITER==1) THEN
          IF (init == 0) THEN
       ...
       ELSE
          ...
          flop=flop+dble(2*N)
       END IF
       CALL ZAXPY(NBLAP, -ALPHA, S(1+N), IONE, f, IONE)
       !
...
!-----------------------------------------------------------------------
  SUBROUTINE zagmg_GCR(N,f,X,ITER,RESID,a,ja,ia,init,flop)
    ...
    NBLAP=N
    !
    mritr=nwrkcum+N*(2*nrst+2)+((nrst+1)*nrst)/2+nrst+N
    ALLOCATE (S(n+1:N*(2*nrst+3)+((nrst+1)*nrst)/2+nrst),R(nwrkcum),fsc(N))
    !
    flop=0.0d0
    kstat=0
    IF (wfo) THEN
    ...
    DO WHILE ( ITER < MAXIT .AND. RESID2 > TOL2BNORM2 )
       ...
       !      Restarting
       IF (itm == m) THEN
          CALL ZTPTRS('U','N','U',m, IONE,S(1+iR+kfree*N),          &
               S(1+iv+kfree*N),m,info)
          IF (irst == 0 .AND. init == 0) THEN
             CALL ZGEMV('N',NBLAP,m,cmplx(1,0,kind(0.0d0)),S(1+ku*N), &
                  NBLAP,S(1+iv+kfree*N),IONE,cmplx(0,0,kind(0.0d0)),&
                  X,IONE)
       ...
       !
       fsc(1:N)=f(1:N)*scald(1:N)
       CALL zagmg_prec_matv(1,f,S(1+(ku+itm)*N),S(1+(kc+itm)*N)  &
            ,flop,R,.FALSE.)
       S(1+(ku+itm)*N:(1+ku+itm)*N)=S(1+(ku+itm)*N:(1+ku+itm)*N)*scald(1:N)
       flop=flop+dble(2*N)
       CALL zagmg_matv(N, S(1+(ku+itm)*N), S(1+(kc+itm)*N),       &
            a, ja, ia, flop,iextL1,dt(1)%lstout,                   &
            dt(1)%ilstout,dt(1)%lstin,dt(1)%ilstin,trans)
       !
       !        Gram-Schmidt
       !
       IF (itm > 0) THEN
          DO i=0,itm-1
             dum(1)=ZDOTC(NBLAP,S(1+(kc+i)*N),IONE,S(1+(kc+itm)*N),IONE)
             bet0=dum(1)
             bet0=bet0/S(1+iR+i+(i*(i+1))/2+kfree*N)
             S(1+iR+i+(itm*(itm+1))/2+kfree*N)=bet0
             CALL ZAXPY(NBLAP,-bet0,S(1+(kc+i)*N),IONE,      &
                  S(1+(kc+itm)*N),IONE)
             flop=flop+dble(4*N)
          END DO
       END IF
       ...
       !
       dum(1)=DZNRM2(NBLAP,S(1+(kc+itm)*N),IONE)**2
       dum(2)=ZDOTC(NBLAP, S(1+(kc+itm)*N), IONE, f, IONE)
       IF (ITER == 1) THEN
          IF (init == 0) THEN
             BNORM2=dble(dum(3))
             RESID2=BNORM2
       ...
       !
       CALL ZAXPY(NBLAP, -bet0, S(1+(kc+itm)*N), IONE, f, IONE)
       flop=flop+dble(6*N)
       !
    ...
    IF (itm >= 0) THEN
       itm1=itm+1
       CALL ZTPTRS('U','N','U',itm1, IONE,S(1+iR+kfree*N),     &
            S(1+iv+kfree*N),m,info)
       IF (irst == 0 .AND. init == 0) THEN
          CALL ZGEMV('N',NBLAP,itm1,cmplx(1,0,kind(0.0d0)),S(1+ku*N), &
               NBLAP,S(1+iv+kfree*N),IONE,cmplx(0,0,kind(0.0d0)),   &
               X,IONE)
...
!--------------------------------------------------------------------
  SUBROUTINE zagmg_matv(n, x, y, a, ja, ia, flop,                &
       iext, lstout, ilstout, lstin, ilstin,transpose)
!
!  Performs matrix vector product by x, result in y
!  If trans==.TRUE., multiply by the transpose
!
    IMPLICIT NONE
    INTEGER :: n, ja(*), ia(n+1), i, kk, k1, k2, ier, idum
    INTEGER :: iext(n), lstout(*), ilstout(*), lstin(0:*), ilstin(*)
    COMPLEX(kind(0.0d0)) :: x(n), y(n), a(*), t
    REAL(kind(0.0d0)) :: flop
    LOGICAL :: transpose
    !
    !
...
!
    ...
    INTEGER :: l
    REAL(kind(0.0d0)) :: flop
    COMPLEX(kind(0.0d0)) ::  B(*), X(*), AX(*), R(*), dum
    LOGICAL :: matv,update
    INTEGER :: is,n,nnext,XN,BN,AXN,RN,idum
!
...
!
    ...
    IF (nnext > 0) THEN
       ...
       IF (l+1 == nlev) THEN
          ...
          CALL zagmg_MUMPSseq(nnext,dt(l+1)%a,dt(l+1)%ja,dt(l+1)%ia,R,2,flop)
          !
       ...
       ELSE
          ...
          CALL zagmg_Galerkin_inner(nnext,R(XN),R(BN),l+1,flop)
          !
       END IF
       !
...
!   stored in (dt(l)%a,dt(l)%ja,dt(l)%iu) (strict upper part)
    ...
    IF (update) THEN
       ...
       !
       CALL  zagmg_csrmv(n,dt(l)%a,dt(l)%ja,n+1,dt(l)%iu,R,1,AX,-1,flop,-1)
       !
    ELSE
       !
       ! Solve (D+E) X = B
       ...
       ! Hence, compute AX = - F * R
       !
       CALL  zagmg_csrmv(n,dt(l)%a,dt(l)%ja,n+1,dt(l)%iu,X,1,AX,-1,flop,-1)
       !
    END IF
    RETURN
...
!
    ...
    !
    AX(1:n)=B(1:n)
    CALL  zagmg_csrmv(n,dt(l)%a,dt(l)%ja,n+1,dt(l)%il,X,1,AX,-2,flop,-1)
    !
    ! Solve (D+F) X = AX
...
!
    ...
    !
    CALL  zagmg_csrmv(n,dt(l)%a,dt(l)%ja,n+1,dt(l)%iu,X,1,AX,0,flop,-1)
    !
    ! Then, R(1:n)= B - AX
    R(1:n)=B(1:n)-AX(1:n)
...
!-----------------------------------------------------------------------
  RECURSIVE SUBROUTINE zagmg_Galerkin_inner(n,X,R,l,flop)
    !
    ...
    !
    dum(1) = ZDOTC(NBLAP,X,IONE,R(1,2),IONE)
    dum(2) = ZDOTC(NBLAP,X,IONE,R,IONE)
    BNORM = dble(dum(3))
    rho1=dum(1)
    alpha1=dum(2)
    !
    ...
    !
    !
    dum(1) = ZDOTC(NBLAP,R(1,3),IONE,R(1,2),IONE)
    dum(2) = ZDOTC(NBLAP,R(1,3),IONE,R,IONE)
    dum(3) = ZDOTC(NBLAP,R(1,3),IONE,R(1,4),IONE)
    IF (.NOT.spd) dum(4) = ZDOTC(NBLAP,X,IONE,R(1,4),IONE)
    gamm0 =dum(1)
    alpha2=dum(2)
    rho2  =dum(3)
    IF (spd) THEN
    ...
    RETURN
    !
  END SUBROUTINE zagmg_Galerkin_inner
!-----------------------------------------------------------------------
...
!
    ...
    IF (1 /= nlev) THEN
       ...
       !
       CALL zagmg_aggregation(1,n,a,ja,ia,nc,listrank)
       !
       ! Perform setup at next level
       !
       CALL zagmg_setup(2,nc,listrank,nc+1)
       !
    ELSE
       !
       ...
       memi=memi-(n+1)
       CALL zagmg_MUMPSseq(n,a,ja,ia,fff,1,ops)
       IF (wfo) THEN
          WRITE(lout,911) IRANK,ops/dble(11*nn(1)+2*nlc1(2))
          call mexPrintf(lout//char(10))
       END IF
       wcplex(4)=0.0d0
    ...
    !
       ...
       !
       CALL zagmg_csrdlu(n,a,ja,ia,dt(1)%idiag,iextL1               &
            ,ap,jap,dt(1)%il,dt(1)%iu,dt(1)%iext,transint)
       IF (transint) THEN
          DEALLOCATE(dt(1)%idiag)
          memi=memi-(n+1)
       ELSE
...
!-----------------------------------------------------------------------
  RECURSIVE SUBROUTINE zagmg_setup(l,n,listrank,ifl)
!
...
!         .OR. ( zerors .AND. ngl(1) <= maxcoarseslowt )  &
    ...
    IF (l /= nlev) THEN
       ...
       ! Coarsening by aggregation
       !
       CALL zagmg_aggregation(l,n,dt(l)%a,dt(l)%ja,dt(l)%ia,nc,listrank)
       !
       ! Perform setup at next level
       !
       CALL zagmg_setup(l+1,nc,listrank,nc+1)
       !
    ELSE
       !
       ! Coarsest level set up
       ...
       memi=memi-(n+1)
       CALL zagmg_MUMPSseq(n,dt(l)%a,dt(l)%ja,dt(l)%ia,fff,1,ops)
       !
    ...
    !
       ...
       memax=MAX(memax,memr+memi*rlenilen)
       !
       CALL zagmg_csrdlu(n,dt(l)%a,dt(l)%ja,dt(l)%ia,dt(l)%idiag,dt(l)%iext &
            ,ap,jap,dt(l)%il,dt(l)%iu,dt(l)%iext,transint)
...
!
    ...
    IF (l .EQ. 1) THEN
       ...
       memi=memi+4*n+1
       memr=memr+n
       CALL zagmg_setCMK(n,ja,ia,dt(l)%idiag,dt(l)%iext,riperm,iperm)
    ELSE
    ...
    memax=MAX(memax,memr+memi*rlenilen)
    !
    call zagmg_prepareagg(n,a,ja,ia,dt(l)%idiag,dt(l)%iext,ind2   &
            ,iperm,si1,ndd,l)
    !
    IF (ndd .EQ. n) THEN
       nc=0
    ...
    IF (l > 1) THEN
       IF (spd) THEN
          CALL zagmg_findpairs_SI(n,a,ja,ia,dt(l)%idiag,dt(l)%iext,si1  &
               ,ind2,lcg,nc,ndd,ldd,skipass                              &
               ,iperm)
       ELSE
          CALL zagmg_findpairs_GI(n,a,ja,ia,dt(l)%idiag,dt(l)%iext,si1  &
               ,ind2,lcg,nc,ndd,ldd,skipass                              &
               ,iperm)
       END IF
       DEALLOCATE(iperm)
    ...
    ELSE
       IF (spd) THEN
          CALL zagmg_findpairs_SI1(n,a,ja,ia,dt(l)%idiag,dt(l)%iext,si1  &
               ,ind2,lcg,nc,ndd,ldd,skipass                               &
               ,riperm,iperm)
       ELSE
          CALL zagmg_findpairs_GI1(n,a,ja,ia,dt(l)%idiag,dt(l)%iext,si1  &
               ,ind2,lcg,nc,ndd,ldd,skipass                               &
               ,riperm,iperm)
       END IF
    ...
    memax=MAX(memax,memr+memi*rlenilen)
    CALL zagmg_setcg(n,a,ja,ia,dt(l)%idiag,dt(l)%iext,si1,ind2,lcg      &
    ...
    DO kpass=2,npass1
       ...
       ind2(1:np)=-1
       IF (spd) THEN
          CALL zagmg_findpairs_SF(np,ap,jap,iap,idiagp,iextp,sip  &
               ,ind2,lcg,nc                                        &
               ,m1,lcg1,a,ja,ia,dt(l)%idiag,dt(l)%iext,si1,w,iw)
       ELSE
          CALL zagmg_findpairs_GF(np,ap,jap,iap,idiagp,iextp,sip     &
               ,ind2,lcg,nc                                           &
               ,m1,lcg1,a,ja,ia,dt(l)%idiag,dt(l)%iext,si1,w,iw)
       END IF
       DEALLOCATE(w,iw)
       ...
       !
       CALL zagmg_setcg(np,ap,jap,iap,idiagp,iextp,sip,ind2,lcg,nc,an     &
...
!-----------------------------------------------------------------------
  SUBROUTINE zagmg_setCMK(n,ja,ia,idiag,iext,riperm,iperm)
    !
...
!-----------------------------------------------------------------------
  SUBROUTINE zagmg_prepareagg(n,a,ja,ia,idiag,iext,ind2,iperm,si,ndd,l)
    !
...
!-----------------------------------------------------------------------
  SUBROUTINE zagmg_findpairs_GF(n,a,ja,ia,idiag,iext,si,ind,lcg,nc     &
...
20     CONTINUE
       ...
       !       in right order
       CALL zagmg_checktentagg_GF(isel,ipair           &
...
!
    ...
    REAL(kind(0.0d0)) :: T
    LOGICAL :: exc
    REAL(kind(0.0d0)), PARAMETER :: bndmum1m1=1.0d0/(kaptg_dampJac-1.0d0)
    REAL(kind(0.0d0)), PARAMETER :: dbndmum1=2.0d0/kaptg_dampJac
    REAL(kind(0.0d0)), PARAMETER :: umdbndmum1=1.0d0-dbndmum1
!
...
!-----------------------------------------------------------------------
  SUBROUTINE zagmg_findpairs_SF(n,a,ja,ia,idiag,iext,si,ind,lcg,nc     &
    ,m1,lcg1,a1,ja1,ia1,idiag1,iext1,si1,rtent,jtent)
! Performs pairwise aggregation according to Algorithms 4.2 and 4.3 in [2,3],
! with some heuristic enhancements to cover non diagonally  dominant matrices.
!
...
20     CONTINUE
       !       Perform check in unagregated matrix, considering possible pairs
       !       in right order
       CALL zagmg_checktentagg_SF(isel,ipair           &
            ,m1,lcg1,a1,ja1,ia1,idiag1,iext1,si1,val,acc)
       !
       IF (.NOT.acc) THEN
          !     Tentative pair has been rejected, check for the next one, if any
...
!
    ...
    REAL(kind(0.0d0)) :: T
    LOGICAL :: exc
    REAL(kind(0.0d0)), PARAMETER :: bndmum1m1=1.0d0/(kaptg_blocdia-1.0d0)
    REAL(kind(0.0d0)), PARAMETER :: dbndmum1=2.0d0/kaptg_blocdia
    REAL(kind(0.0d0)), PARAMETER :: umdbndmum1=1.0d0-dbndmum1
!
! Find indices of submatrix to be extracted and store them in set(1:setdim)
    IF (m1.eq.2) THEN
...
!-----------------------------------------------------------------------
  SUBROUTINE zagmg_findpairs_GI(n,a,ja,ia,idiag,iext,si,ind,lcg,nc     &
    ,nddl,ldd,skipass                                      &
    ,ipc)
! Performs pairwise aggregation according to Algorithms 4.2 and 4.3 in [2,3],
! with some heuristic enhancements to cover non diagonally  dominant matrices.
...
!-----------------------------------------------------------------------
  SUBROUTINE zagmg_findpairs_SI(n,a,ja,ia,idiag,iext,si,ind,lcg,nc     &
    ,nddl,ldd,skipass                                      &
    ,ipc)
! Performs pairwise aggregation according to Algorithms 4.2 and 4.3 in [2,3],
! with some heuristic enhancements to cover non diagonally  dominant matrices.
...
!-----------------------------------------------------------------------
  SUBROUTINE zagmg_findpairs_GI1(n,a,ja,ia,idiag,iext,si,ind,lcg,nc     &
    ,nddl,ldd,skipass                                      &
    ,riperm,iperm)
! Performs pairwise aggregation according to Algorithms 4.2 and 4.3 in [2,3],
! with some heuristic enhancements to cover non diagonally  dominant matrices.
...
!-----------------------------------------------------------------------
  SUBROUTINE zagmg_findpairs_SI1(n,a,ja,ia,idiag,iext,si,ind,lcg,nc     &
    ,nddl,ldd,skipass                                      &
    ,riperm,iperm)
! Performs pairwise aggregation according to Algorithms 4.2 and 4.3 in [2,3],
! with some heuristic enhancements to cover non diagonally  dominant matrices.
...
!-----------------------------------------------------------------------
  SUBROUTINE zagmg_setcg(n,a,ja,ia,idiag,iext,si,ind,lcg           &
...
!!!!!!!!!!!!!!!!!!! ROUTINES FOR DIRECT SOLUTION
  !-----------------------------------------------------------------------
  SUBROUTINE zagmg_LAPACK(N,a,ja,ia,f,ijb,flop)
    USE zagmg_mem
    IMPLICIT NONE
    INTEGER :: N,ia(n+1),ja(*),ijb
    COMPLEX(kind(0.0d0)) :: a(*),f(n)
    REAL(kind(0.0d0)) :: flop
    !
  ...
  !-----------------------------------------------------------------------
  SUBROUTINE zagmg_MUMPSseq(n,a,ja,ia,f,ijob,flop)
    USE zagmg_mem
    IMPLICIT NONE
!!!!!!!! BEGIN Include ZAGMG_MUMPS definitions !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!
!
...
!!!!!!!! --END Include ZAGMG_MUMPS definitions !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    INTEGER :: n,ia(n+1),ijob
    INTEGER, TARGET :: ja(*)
    COMPLEX(kind(0.0d0)), TARGET :: a(*),f(n)
    REAL(kind(0.0d0)) :: flop
    !
    INTEGER, SAVE :: iflop
