...
int main(int argc, char **argv)
{
    ...
    integer      i,j,k,l,m,fnamelen,n,nc,nz,nrhs,tmp0,tmp,tmp2,tmp3,ierr,nLU,
                 nrhsix, *rhsptr, *rhsind, nnzL,nnzU, nAx=0, 
                 mynrhs=2, transpose=-1, sumit;
    REALS        DROP_TOL, CONDEST, ELBOW, val,vb;
    FLOAT        *rhs,*sol, *leftscale, *rightscale, *rhsval;
    ...
    // get file name of the source matrix
    i=0;
...
#include "../../../ilupack/samples/readmatrix.c"
    ...
    // interchange order of scalings for all three cases
    // first column scaling, then row scaling
    param.ipar[7]|=4+32+256;
...
#include "../../../ilupack/samples/initvectors.c"
	...
	/* left  preconditionig:                   param.ipar[21]=1
	   ...
	   L^{-1} as left preconditioner and 
           (DU)^{-1} as right preconditioner:      param.ipar[21]=2
	*/
	// transposed system is stored
	param.ipar[4]=1; 
	ierr=AMGSOLVER(&AT, &PRE, &param, rhs+A.nr*l, sol+A.nr*l);
	evaluate_time(&time_stop,&systime);
	secnds=time_stop-time_start;
...
#include "../../../ilupack/samples/finalres.c"
    ...
    WRITEVECTORS(fname,sol,&(AT.nr),&mynrhs,
    ...
    /*
      ...
      READVECTORS(fname,sol,&(AT.nr),&mynrhs,title,key,j, 72, 8);
      title[72]='\0';
      key[8]='\0';
    */

    // finally release memory of the preconditioner
    AMGDELETE(&AT,&PRE,&param);
