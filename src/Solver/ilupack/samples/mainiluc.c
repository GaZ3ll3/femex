...
|---------------------------------------------------------------------*/
    ...
    */
    ...
    ilutmat.ja=(integer *)  MALLOC((size_t)ipar[1]*sizeof(integer),  "mainilutp:ilutmat.ja");
    ilutmat.a =(FLOAT *)MALLOC((size_t)ipar[1]*sizeof(FLOAT),"mainilutp:ilutmat.a");
    ...
    ws=(integer *)  MALLOC(9*(size_t)n*sizeof(integer),  "mainilutp:ws");
    w =(FLOAT *)MALLOC(5*(size_t)n*sizeof(FLOAT),"mainilutp:w");


    /* perform ILUT decomposition */
    /* DROP_INVERSE|TISMENETSKY_SC|NO_SHIFT|REPEAT_FACT|IMPROVED_ESTIMATE */
    /* flags=DROP_INVERSE|IMPROVED_ESTIMATE; */
    //flags=DROP_INVERSE|IMPROVED_ESTIMATE|DIAGONAL_COMPENSATION|NO_SHIFT;
    flags=DROP_INVERSE|IMPROVED_ESTIMATE;

    printf("ILUC PARAMETERS:\n");
    printf("   droptol=%8.1e\n",DROP_TOL);
    printf("   lfil   =%d\n",  ipar[0]);
    ...
    evaluate_time(&time_start,&systime);
    ILUC(&n,A.a,A.ja,A.ia,ipar,fpar,&flags,
	 ilutmat.a,ilutmat.ja,ilutmat.ia,ipar+1,w,ws,ipar+2);
    evaluate_time(&time_stop,&systime);
    ...
    w=(FLOAT *)MALLOC((size_t)i*sizeof(FLOAT),"mainiluc:w"); 
    ipar[3]=i;
...
*/
	...
	i=1;
	if (ipar[1]==1)
	   ILUCSOL(&n, w+ipar[7],w+ipar[8], ilutmat.a,ilutmat.ja,ilutmat.ia);
	else
	...
	i=1;
	if (ipar[1]==1) 
	   ILUCTSOL(&n, w+ipar[7],w+ipar[8], ilutmat.a,ilutmat.ja,ilutmat.ia);
	else
	   COPY(&n,w+ipar[7],&i,w+ipar[8],&i);
	break;
