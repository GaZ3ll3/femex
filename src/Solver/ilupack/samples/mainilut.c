...
|---------------------------------------------------------------------*/
    ...
    */
    ...
    ws=(integer *)  MALLOC(5*(size_t)n*sizeof(integer),"mainilut:ws");
    w =(FLOAT *)MALLOC((size_t)n*sizeof(FLOAT),"mainilut:w");


    /* perform ILUT decomposition */

    printf("ILUT2 PARAMETERS:\n");
    printf("   droptol=%8.1e\n",DROP_TOL);
    printf("   lfil   =%d\n",  ipar[0]);
    ...
    evaluate_time(&time_start,&systime);
    ILUT(&n,A.a,A.ja,A.ia,ipar,fpar,
...
*/
	...
	i=1;
	if (ipar[1]==1)
	   LUSOL(&n, w+ipar[7],w+ipar[8], ilutmat.a,ilutmat.ja,ilutmat.ia);
	else
	...
	i=1;
	if (ipar[1]==1) 
	   LUTSOL(&n, w+ipar[7],w+ipar[8], ilutmat.a,ilutmat.ja,ilutmat.ia);
	else
	   COPY(&n,w+ipar[7],&i,w+ipar[8],&i);
	break;
