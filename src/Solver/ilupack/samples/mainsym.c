...
int main(int argc, char **argv)
{
    ...
    integer      i,j,k,l,m,fnamelen,n,nc,nz,nrhs,tmp0,tmp,tmp2,tmp3,ierr,
                 nrhsix, *rhsptr, *rhsind, nnzU, nAx=0, mynrhs=2, sumit, *ptr;
    REALS        DROP_TOL, CONDEST, ELBOW, val,vb;
    FLOAT        *rhs,*sol, *scale, *rhsval, *mytestvector;
    float        systime, time_start, time_stop, secnds, sumtime;
    AMGLEVELMAT  PRE, *next;
    ILUPACKPARAM param;

    // some internal variables when using mixed precision
    // used for displaying the performance
...
#include "../../../ilupack/samples/spdreadmatrix.c"
    ...
    // use the drop tolerance as for the LU factors but multiplied by .1
    param.droptolS=0.1*DROP_TOL;
    ...
    // 19. indicate if you have a saddle point structure
    // in this case pass a vector of length n. All negative components
    // indicate the second lower right block of
    // [ A  B]
    // [B^T C]
    // It is not necessary that the saddle point structure is 
    // preordered in a 2x2 block matrix
    param.ind=NULL;
    /*
      param.ind=(integer *) MALLOC (A.nr*sizeof(integer),"mainsym:ind");
      for (i=0; i<A.nr; i++)
          param.ind[i]=1;
      for (i=A.nr; i<A.nr; i++)
          param.ind[i]=-1;
    ...
    // 20. require the computation of the preconditioner in
    //     single precision
...
#include "../../../ilupack/samples/symmessages.c"
    ...
    if (!strcmp(param.typetv,"static") || !strcmp(param.typetv,"none")) {
       // the standard way of calling the ILU factorization
       ierr=MYSYMAMGFACTOR(&A, &PRE, &param);
    }
    else {
       // a more advanced version 
       // before entering for the first time init return label
       // param.returnlabel=0;
       // MYSYMAMGINIT will do this automatically
       do {
	  ierr=MYSYMAMGFACTOR(&A, &PRE, &param);
	  if (param.rcomflag!=0 && ierr==0) {
	     // Here we could manipulate the test vector
	  }
       } 
       while (ierr==0 && param.rcomflag!=0);
    }
