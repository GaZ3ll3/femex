...
/*
    ...
    [x, options] = DGNLSYMilupacksolver(A,PREC,options, b,x0);
...
)
{
    ...
    A.nc=A.nr=mrows;
    A.ia=(integer *) MAlloc((size_t)(A.nc+1)*sizeof(integer),"DGNLSYMilupacksolver");
    A.ja=(integer *) MAlloc((size_t)nnz     *sizeof(integer),"DGNLSYMilupacksolver");
    A. a=(double  *) MAlloc((size_t)nnz     *sizeof(double), "DGNLSYMilupacksolver");
    ...
    */
	...
	if (classIDflags[ifield] == mxCHAR_CLASS) {
	   ...
	   input_buf = (char *) mxCalloc((size_t)buflen, sizeof(char));
    ...
    /* get size of input matrix A */
    rhs=(double*) MAlloc((size_t)A.nr*sizeof(double),"DGNLSYMilupacksolver:rhs");
    pr=mxGetPr(b_input);
    ...
    /* numerical solution */
    sol=(double *)MAlloc((size_t)A.nr*sizeof(double),"DGNLSYMilupacksolver:sol");
    pr=mxGetPr(x0_input);
    ...
    /* set bit 3, conjugate */
    param->ipar[4]|=0;
    ierr=DGNLSYMAMGsolver(&A, PRE, param, rhs, sol);
    ...
    *pr=param->ipar[26];
    ...
    x_output=plhs[0];
    pr=mxGetPr(x_output);
    memcpy(pr,sol,A.nr*sizeof(double));  
