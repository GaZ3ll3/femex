/* ========================================================================== */
/* === mwmilupackrcm mexFunction ============================================ */
/* ========================================================================== */

/*
    Usage:

    Return Reverse Cuthill-McKee reordering combined
    with maximum weight matching
    
    Example:

    % for initializing parameters
    [pl,pr,Dl,Dr] = mwmilupackrcm(A);
...
)
{
    ...
    A.nc=A.nr=mrows;
    A.ia=(integer *)MAlloc((size_t)(A.nc+1)*sizeof(integer),"mwmilupackrcm");
    A.ja=(integer *)MAlloc((size_t)nnz     *sizeof(integer),"mwmilupackrcm");
    A.a =(double *) MAlloc((size_t)nnz     *sizeof(double), "mwmilupackrcm");
    A.ia[0]=1;
    for (i = 0 ; i < ncols ; i++) {
        A.ia[i+1]=A.ia[i];
    ...
    p   =(integer *) MAlloc((size_t)A.nc*sizeof(integer),"mwmilupackrcm");
    invq=(integer *) MAlloc((size_t)A.nc*sizeof(integer),"mwmilupackrcm");
    prowscale=(double *) MAlloc((size_t)A.nc*sizeof(double),"mwmilupackrcm");
    pcolscale=(double *) MAlloc((size_t)A.nc*sizeof(double),"mwmilupackrcm");

#ifdef _MC64_MATCHING_    
    ierr=DGNLperm_mc64_rcm(A, prowscale, pcolscale, p, invq, &nB, &param);
#elif defined _PARDISO_MATCHING_
    ierr=DGNLperm_mwm_rcm(A, prowscale, pcolscale, p, invq, &nB, &param);
#else /* MUMPS matching */
    ierr=DGNLperm_matching_rcm(A, prowscale, pcolscale, p, invq, &nB, &param);
#endif
