/* ========================================================================== */
/* === mwmilupackmetisn mexFunction ============================================ */
/* ========================================================================== */

/*
    Usage:

    Return METIS Nested Dissection by nodes reordering combined
    with maximum weight matching
    
    Example:

    % for initializing parameters
    [pl,pr,Dl,Dr] = mwmilupackmetisn(A);
...
)
{
    ...
    A.nc=A.nr=mrows;
    A.ia=(integer *)MAlloc((size_t)(A.nc+1)*sizeof(integer),"mwmilupackmetisn");
    A.ja=(integer *)MAlloc((size_t)nnz     *sizeof(integer),"mwmilupackmetisn");
    A.a =(double *) MAlloc((size_t)nnz     *sizeof(double), "mwmilupackmetisn");
    A.ia[0]=1;
    for (i = 0 ; i < ncols ; i++) {
        A.ia[i+1]=A.ia[i];
    ...
    p   =(integer *) MAlloc((size_t)A.nc*sizeof(integer),"mwmilupackmetisn");
    invq=(integer *) MAlloc((size_t)A.nc*sizeof(integer),"mwmilupackmetisn");
    prowscale=(double *) MAlloc((size_t)A.nc*sizeof(double),"mwmilupackmetisn");
    pcolscale=(double *) MAlloc((size_t)A.nc*sizeof(double),"mwmilupackmetisn");

#ifdef _MC64_MATCHING_    
    ierr=DGNLperm_mc64_metis_n(A, prowscale, pcolscale, p, invq, &nB, &param);
#elif defined _PARDISO_MATCHING_
    ierr=DGNLperm_mwm_metis_n(A, prowscale, pcolscale, p, invq, &nB, &param);
#else /* MUMPS matching */
    ierr=DGNLperm_matching_metis_n(A, prowscale, pcolscale, p, invq, &nB, &param);
#endif
