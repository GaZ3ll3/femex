...
)
{
    ...
    */
	...
	for (j=A_ia[i]; j<A_ia[i+1]; j++) {
	    ...
	    if (k>=i) {
	       ...
	       A.ja[l]=k+1;
	       A.a [l].r=A_valuesR[j];
	       A.a [l].i=A_valuesI[j]; /* lower triangular part correct */
	       A.ia[i+1]=l+2;
	    }
	}
    }
    ...
    */
    ...
    rhs=(doublecomplex*)MAlloc((size_t)i*sizeof(doublecomplex),"ZHERsavehbo:rhs");
    sol=rhs;
    tname[0]=' ';
    tname[1]=' ';
    tname[2]=' ';
    ...
    Zwritemtc(fname,A.a,A.ja,A.ia,rhs,&ncolumns,
	      tname,&(A.nr),&(A.nc),&m,
	      "ILUPACK toolbox for MATLAB. Export routine to Harwell-Boeing format     ",fname2,"cha",strlen(fname),3,72,8,3);
