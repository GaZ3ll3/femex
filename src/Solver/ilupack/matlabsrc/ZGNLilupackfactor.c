...
)
{
    ...
    */
    ...
    istack=(integer *)MAlloc((size_t)A.nr*sizeof(integer),    "ZGNLilupackfactor:istack");
	...
	if (jstruct<PRE->nlev-1){ 
 	   ...
 	   if (param->flags&COARSE_REDUCE) {
		 ...
		 for (i=0; i<scurrent->E.nr; i++) {
		     ...
		     Cqsort(scurrent->E.a+j,scurrent->E.ja+j,istack,&jj);
		 ...
		 for (i=0; i<current->E.nr; i++) {
		     ...
		     Zqsort(current->E.a+j,current->E.ja+j,istack,&jj);
