...
)
{
    ...
    mwSize     mrows, ncols, ndim, buflen;
    int        nfields, ifield, status;
    size_t     sizebuf;
    double     dbuf, *pr, *pi;
    ...
    nlhs=1;
    plhs[0] = mxCreateStructMatrix((mwSize)1, (mwSize)1, (mwSize)nfields, fnames);
	...
	if (classIDflags[ifield] == mxCHAR_CLASS) {
	   ...
	   if (!strcmp("elbow",fnames[ifield])) {
	      ...
	      fout=mxCreateDoubleMatrix((mwSize)1,(mwSize)1, mxCOMPLEX);
	      pr=mxGetPr(fout);
	      pi=mxGetPi(fout);
	      *pr=param.damping.r;
	      *pi=param.damping.i;
