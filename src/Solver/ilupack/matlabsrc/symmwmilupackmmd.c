...
)
{
    ...
    A.nc=A.nr=mrows;
    A.ia=(integer *)MAlloc((size_t)(A.nc+1)*sizeof(integer),"symmwmilupackmmd");
    A.ja=(integer *)MAlloc((size_t)nnz     *sizeof(integer),"symmwmilupackmmd");
    A.a =(double *) MAlloc((size_t)nnz     *sizeof(double), "symmwmilupackmmd");
    A.ia[0]=1;
    for (i = 0 ; i < ncols ; i++) {
        A.ia[i+1]=A.ia[i];
    ...
    p   =(integer *) MAlloc((size_t)A.nc*sizeof(integer),"symmwmilupackmmd");
    invq=(integer *) MAlloc((size_t)A.nc*sizeof(integer),"symmwmilupackmmd");
    prowscale=(double *) MAlloc((size_t)A.nc*sizeof(double),"symmwmilupackmmd");
    pcolscale=prowscale;
#ifdef _MC64_MATCHING_    
    ierr=DSYMperm_mc64_mmd(A, prowscale, pcolscale, p, invq, &nB, &param);
#elif defined _PARDISO_MATCHING_
    ierr=DSYMperm_mwm_mmd(A, prowscale, pcolscale, p, invq, &nB, &param);
#else /* MUMPS matching */
    ierr=DSYMperm_matching_mmd(A, prowscale, pcolscale, p, invq, &nB, &param);
#endif
