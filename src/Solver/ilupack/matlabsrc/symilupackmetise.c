...
)
{
    ...
    A.nc=A.nr=mrows;
    A.ia=(integer *) MAlloc((size_t)(A.nc+1)*sizeof(integer),"symilupackmetise");
    A.ja=(integer *) MAlloc((size_t)nnz     *sizeof(integer),"symilupackmetise");
    A.a=NULL;
    A.ia[0]=1;
    for (i = 0 ; i < ncols ; i++) {
        A.ia[i+1]=A.ia[i];
    ...
    p   =(integer *) MAlloc((size_t)A.nc*sizeof(integer),"symilupackmetise");
    invq=(integer *) MAlloc((size_t)A.nc*sizeof(integer),"symilupackmetise");
    ierr=DGNLperm_metis_e(A, prowscale, pcolscale, p, invq, &nB, &param);
