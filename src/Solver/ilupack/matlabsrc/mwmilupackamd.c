/* ========================================================================== */
/* === mwmilupackamd mexFunction ============================================ */
/* ========================================================================== */

/*
    Usage:

    Return AMD reordering combined with maximum weight matching
    
    Example:

    % for initializing parameters
    [pl,pr,Dl,Dr] = mwmilupackamd(A);
...
)
{
    ...
    A.nc=A.nr=mrows;
    A.ia=(integer *)MAlloc((size_t)(A.nc+1)*sizeof(integer),"mwmilupackamd");
    A.ja=(integer *)MAlloc((size_t)nnz     *sizeof(integer),"mwmilupackamd");
    A.a =(double *) MAlloc((size_t)nnz     *sizeof(double), "mwmilupackamd");
    A.ia[0]=1;
    for (i = 0 ; i < ncols ; i++) {
        A.ia[i+1]=A.ia[i];
    ...
    p   =(integer *) MAlloc((size_t)A.nc*sizeof(integer),"mwmilupackamd");
    invq=(integer *) MAlloc((size_t)A.nc*sizeof(integer),"mwmilupackamd");
    prowscale=(double *) MAlloc((size_t)A.nc*sizeof(double),"mwmilupackamd");
    pcolscale=(double *) MAlloc((size_t)A.nc*sizeof(double),"mwmilupackamd");

#ifdef _MC64_MATCHING_    
    ierr=DGNLperm_mc64_amd(A, prowscale, pcolscale, p, invq, &nB, &param);
#elif defined _PARDISO_MATCHING_
    ierr=DGNLperm_mwm_amd(A, prowscale, pcolscale, p, invq, &nB, &param);
#else /* MUMPS matching */
    ierr=DGNLperm_matching_amd(A, prowscale, pcolscale, p, invq, &nB, &param);
#endif
