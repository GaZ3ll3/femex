/* ========================================================================== */
/* === mwmilupackmmd mexFunction ============================================ */
/* ========================================================================== */

/*
    Usage:

    Return Minimum Degree reordering combined
    with maximum weight matching
    
    Example:

    % for initializing parameters
    [pl,pr,Dl,Dr] = mwmilupackmmd(A);
...
)
{
    ...
    A.nc=A.nr=mrows;
    A.ia=(integer *)MAlloc((size_t)(A.nc+1)*sizeof(integer),"mwmilupackmmd");
    A.ja=(integer *)MAlloc((size_t)nnz     *sizeof(integer),"mwmilupackmmd");
    A.a =(double *) MAlloc((size_t)nnz     *sizeof(double), "mwmilupackmmd");
    A.ia[0]=1;
    for (i = 0 ; i < ncols ; i++) {
        A.ia[i+1]=A.ia[i];
    ...
    p   =(integer *) MAlloc((size_t)A.nc*sizeof(integer),"mwmilupackmmd");
    invq=(integer *) MAlloc((size_t)A.nc*sizeof(integer),"mwmilupackmmd");
    prowscale=(double *) MAlloc((size_t)A.nc*sizeof(double),"mwmilupackmmd");
    pcolscale=(double *) MAlloc((size_t)A.nc*sizeof(double),"mwmilupackmmd");

#ifdef _MC64_MATCHING_    
    ierr=DGNLperm_mc64_mmd(A, prowscale, pcolscale, p, invq, &nB, &param);
#elif defined _PARDISO_MATCHING_
    ierr=DGNLperm_mwm_mmd(A, prowscale, pcolscale, p, invq, &nB, &param);
#else /* MUMPS matching */
    ierr=DGNLperm_matching_mmd(A, prowscale, pcolscale, p, invq, &nB, &param);
#endif
