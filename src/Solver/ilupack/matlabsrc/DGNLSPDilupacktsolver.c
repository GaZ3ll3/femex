...
/*
    ...
    [x, options] = DGNLSPDilupacksolver(A,PREC,options, b,x0);
...
)
{
    ...
    mxArray    *tmp, *fout, *A_input , *PRE_input, *options_input, 
               *b_input, *x0_input, *x_output, *options_output;
    ...
    A.nc=A.nr=mrows;
    A.ia=(integer *) MAlloc((size_t)(A.nc+1)*sizeof(integer),"DGNLSPDilupacksolver");
    A.ja=(integer *) MAlloc((size_t)nnz     *sizeof(integer),"DGNLSPDilupacksolver");
    A. a=(double  *) MAlloc((size_t)nnz     *sizeof(double), "DGNLSPDilupacksolver");
    ...
    */
    ...
    /* get size of input matrix A */
    rhs=(double*) MAlloc((size_t)A.nr*sizeof(double),"DGNLSPDilupacksolver:rhs");
    pr=mxGetPr(b_input);
    ...
    /* numerical solution */
    sol=(double *)MAlloc((size_t)A.nr*sizeof(double),"DGNLSPDilupacksolver:sol");
    pr=mxGetPr(x0_input);
    ...
    /* set bit 3, conjugate */
    param->ipar[4]|=0;
    ierr=DGNLSPDAMGsolver(&A, PRE, param, rhs, sol);
    ...
    if (j==-1)
       ...
       ifield=j;
    fout=mxCreateDoubleMatrix((mwIndex)1,(mwIndex)1, mxREAL);
    pr=mxGetPr(fout);
