...
)
{
    ...
    A.nc=A.nr=mrows;
    A.ia=(integer *)MAlloc((size_t)(A.nc+1)*sizeof(integer),"mwmilupacknull");
    A.ja=(integer *)MAlloc((size_t)nnz     *sizeof(integer),"mwmilupacknull");
    A.a =(double *) MAlloc((size_t)nnz     *sizeof(double), "mwmilupacknull");
    A.ia[0]=1;
    for (i = 0 ; i < ncols ; i++) {
        A.ia[i+1]=A.ia[i];
    ...
    p   =(integer *) MAlloc((size_t)A.nc*sizeof(integer),"mwmilupacknull");
    invq=(integer *) MAlloc((size_t)A.nc*sizeof(integer),"mwmilupacknull");
    prowscale=(double *) MAlloc((size_t)A.nc*sizeof(double),"mwmilupacknull");
    pcolscale=(double *) MAlloc((size_t)A.nc*sizeof(double),"mwmilupacknull");

#ifdef _MC64_MATCHING_    
    ierr=DGNLperm_mc64_null(A, prowscale, pcolscale, p, invq, &nB, &param);
#elif defined _PARDISO_MATCHING_
    ierr=DGNLperm_mwm_null(A, prowscale, pcolscale, p, invq, &nB, &param);
#else /* MUMPS matching */
    ierr=DGNLperm_matching_null(A, prowscale, pcolscale, p, invq, &nB, &param);
#endif
