...
)
{
    ...
    static double  fpar[16], *workspace;
    static int init=0;
    ...
    pr=mxGetPr(b_input);
    n=mxGetM(b_input);
    rhs=(double *) MAlloc((size_t)n*sizeof(double), "DSYMilupacksqmr");
    for (i=0; i<n; i++) {
        rhs[i]=pr[i];
    }
    ...
    pr=mxGetPr(b_input);
    sol=(double *) MAlloc((size_t)n*sizeof(double), "DSYMilupacksqmr");
    for (i=0; i<n; i++) {
        sol[i]=pr[i];
    }
    ...
    if (!init) {
	 ...
	 fpar[11]=fpar[12]=fpar[13]=fpar[14]=fpar[15]=0.0;
       ...
       /* provide workspace */
       workspace=(double *) MAlloc(6*(size_t)n*sizeof(double),"DSYMilupacksqmr");
    }
    ...
    m=n;
    DSYMqmr(&m,rhs,sol,ipar,fpar,workspace);
