...
/*
    ...
    Return the structure 'options' and solution x of Ax=b based on ILUPACK V2.3
    
    Example:

    % for initializing parameters
    [x, options] = ZGNLilupacksolver(A,PREC,options, b,x0);
...
)
{
    ...
    A.nc=A.nr=mrows;
    A.ia=(integer *) MAlloc((size_t)(A.nc+1)*sizeof(integer),"ZGNLilupacksolver");
    A.ja=(integer *) MAlloc((size_t)nnz     *sizeof(integer),"ZGNLilupacksolver");
    A. a=(doublecomplex *) MAlloc((size_t)nnz     *sizeof(doublecomplex), "ZGNLilupacksolver");
    ...
    */
    ...
    /* mexPrintf("read preconditioning matrix\n");fflush(stdout); */
    ...
    */
    ...
    /* numerical solution */
    sol=(doublecomplex *)MAlloc((size_t)A.nr*sizeof(doublecomplex),"ZGNLilupacksolver:sol");
    pr=mxGetPr(x0_input);
    ...
    /* mexPrintf("start iterative solver\n");fflush(stdout); */
    ierr=ZGNLAMGsolver(&A, PRE, param, rhs, sol);
    /* mexPrintf("iterative solver finished with ierr =%d\n",ierr);fflush(stdout); */
