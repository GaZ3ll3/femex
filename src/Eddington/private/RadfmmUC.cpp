...
double kernel_RadfmmUC::kernel_Func(Point r0, Point r1){
    double rSquare	= distance(r0.x, r0.y, r1.x, r1.y) ;
    if (rSquare == 0){
    	return 0.;
    }
    else{
        return exp(-this->integral(r0.x, r0.y, r1.x, r1.y)) * (r0.y - r1.y)/rSquare/rSquare/(2*M_PI)/side/side;
    }
}

double kernel_RadfmmUC::getAttribute(double x, double y) {
...
template class mexplus::Session<kernel_RadfmmUC>;
...
MEX_DEFINE(new)(int nlhs, mxArray* plhs[], int nrhs, const mxArray* prhs[]){
	...
	output.set(0, Session<kernel_RadfmmUC>::create(new kernel_RadfmmUC()));
...
MEX_DEFINE(delete) (int nlhs, mxArray* plhs[], int nrhs, const mxArray* prhs[]) {
	...
	Session<kernel_RadfmmUC>::destroy(input.get(0));
}
...
MEX_DEFINE(calc_cache) (int nlhs, mxArray* plhs[], int nrhs, const mxArray* prhs[]) {
	...
	auto kern = Session<kernel_RadfmmUC>::get(input.get(0));
